...
namespace Draw3DSurface
{
    public class SurfacePlotVisual3D : ModelVisual3D
    {
        ...
        private Model3D CreateModel()
        {
            ...
            Point3D[,] pts = (Point3D[,])Points.Clone();

            for (int i = 0; i < pts.GetLength(0); i++)
            {
                for (int w = 0; w < pts.GetLength(1); w++)
                {
                    pts[i, w].Z = pts[i, w].Z * Zrate;
                }
            }

            var surfaceMeshBuilder = new MeshBuilder();
            surfaceMeshBuilder.AddRectangularMesh(pts, texcoords);
            ...
            for (double x = minX; x <= maxX; x += IntervalX)
            {
                ...
                for (int i = 0; i < rows; i++)
                {
                    Point3D pt= BilinearInterpolation(Points, i, j);
                    pt.Z = pt.Z * Zrate;
                    path.Add(pt);
                }
                path.Add(new Point3D(x, maxY, minZ * Zrate));
            ...
            for (double y = minY; y <= maxY; y += IntervalY)
            {
                ...
                for (int j = 0; j < columns; j++)
                {
                    Point3D pt3 = BilinearInterpolation(Points, i, j);
                    pt3.Z = pt3.Z * Zrate;
                    path.Add(pt3);
                }
                path.Add(new Point3D(maxX, y, minZ * Zrate));
            ...
            for (double z = z0; z <= maxZ + double.Epsilon; z += IntervalZ)
            {
                GeometryModel3D label = TextCreator.CreateTextLabelModel3D(z.ToString(), Brushes.Black, true, FontSize,
                                                                           new Point3D(minX - FontSize*3, maxY, z * Zrate),
            ...
            var bb = new Rect3D(minX, minY, minZ, maxX - minX, maxY - minY, Zrate * (maxZ - minZ));
